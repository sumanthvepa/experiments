#!/usr/bin/env bash
# -------------------------------------------------------------------
# packages2.sh: Script to create/upgrade all python package
...
# Packages2.sh is a shell script that should be run in a fresh
# python virtual environment to install the latest versions
# of all packages required by 01-remote-idea-python

# Unlike packages.sh, packages2.sh uses the current working
# directory to look for the virtual environment it needs
...
install_local_packages() {
  ...
  if [ ${#local_pkgs[@]} -eq 0 ]; then
    echo "No local packages provided to install."
    return 1
  fi
  # echo "Installing packages in editable mode: ${local_pkgs[*]}"
  python3 -m pip install -e "${local_pkgs[@]}"
}


# Look for virtual environments in the working dir
WORKING_DIR=$(pwd)
...
if [[ ! -v VIRTUAL_ENV ]]; then
  ...
  if [[ $VIRTUAL_ENV != "$WORKING_DIR/venv" ]]; then
...
read_packages "$WORKING_DIR/packages.txt" packages
echo "Installing additional packages: ${packages[*]}"
install_packages "${packages[@]}"

# Install local packages in editable mode
read_packages "$WORKING_DIR/local-packages.txt" local_packages
echo "Installing local packages in editable mode: ${local_packages[*]}"
